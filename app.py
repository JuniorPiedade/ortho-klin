...
st.set_page_config(page_title="OrthoKlin | Gestão Inteligente", layout="wide")
...
if not st.session_state['logado']:
    ...
    with col:
        ...
        if st.session_state['auth_mode'] == "login":
            ...
            if col_b1.button("CRIAR PERFIL"): st.session_state['auth_mode'] = "cadastro"; st.rerun()
            if col_b2.button("PERDI A SENHA"): st.session_state['auth_mode'] = "recuperar"; st.rerun()

        # MODO CADASTRO COM REDIRECIONAMENTO DIRETO
            ...
            if st.button("FINALIZAR E ENTRAR"):
                if nu and np == npc and len(np) > 3:
                    # Armazena temporariamente (para esta sessão) e loga
                    st.session_state['novo_u'] = nu
                    st.session_state['novo_p'] = np
                    st.session_state['logado'] = True # <--- REDIRECIONAMENTO AQUI
                    st.success("Perfil criado! Entrando...")
                    st.rerun()
                else: st.warning("Dados inválidos ou senhas diferentes.")
        ...
        # MODO RECUPERAR
            ...
            em = st.text_input("USUÁRIO OU E-MAIL")
            if st.button("ENVIAR INSTRUÇÕES"): st.info("Verifique seu e-mail.")
            if st.button("VOLTAR"): st.session_state['auth_mode'] = "login"; st.rerun()
...
# --- ÁREA INTERNA ---
    ...
    if st.session_state['menu'] == "Dash":
        ...
        c1.markdown(f'<div class="glass-card"><small>VALOR TOTAL</small><br><b>R$ {df["Valor"].sum():,.2f}</b></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="glass-card"><small>LEADS</small><br><b>{len(df)}</b></div>', unsafe_allow_html=True)
        c3.markdown(f'<div class="glass-card"><small>HOJE</small><br><b>{len(df[df["Data_Retorno"] == date.today()])}</b></div>', unsafe_allow_html=True)
        
        if not df.empty:
            fig = px.bar(df.groupby('Data_Cadastro')['Valor'].sum().reset_index(), x='Data_Cadastro', y='Valor')
    ...
    # GESTÃO
        ...
        for idx, row in df_f.iterrows():
            with st.container():
                st.markdown(f'<div class="glass-card"><b>{row["Nome"].upper()}</b> | <span class="tag-pendente">{row["Status"]}</span><br><small>Retorno: {row["Data_Retorno"]}</small></div>', unsafe_allow_html=True)
                if st.button("APAGAR", key=f"r_{idx}"):
    ...
    # NOVO
        ...
        with st.form("add"):
            ...
            if st.form_submit_button("SALVAR"):
                new = {'Nome': n, 'CPF': '000', 'Telefone': '000', 'Origem': 'Instagram', 'Status': 'Pendente', 'Valor': v, 'Data_Cadastro': date.today(), 'Data_Retorno': dr}
